...
const MetalsSpreadsMobile = ({ data }) => {
  ...
  // Filter data based on search term only (same as desktop version)
  // This ensures mobile shows all 5 metals: XAGUSD, XAGEUR, XAUEUR, XAUUSD, XPTUSD
  const filteredData = data.filter((item) =>
    item.col1.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <div className="metals-spreads-mobile">
      {/* Search Bar */}
...
MetalsSpreadsMobile.propTypes = {
  data: PropTypes.array.isRequired,
};

export default MetalsSpreadsMobile;
